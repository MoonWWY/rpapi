<?php 
	...
	if ($size > 100*1024*1024){
		//
		echo "<script>alert('文件大小超过100M大小');window.history.go(-1);</script>";
		exit();
	}
	...
	//设置允许上传文件的后缀
/*	$allow_suffix = array('jpg','gif','jpeg','png','zip');
	...
	$uploadstime = date('Ymd',time());
	if (!file_exists('uploads/'.$uploadstime)){
	    mkdir('uploads/'.$uploadstime);
	}
	$time_dir = 'uploads/'.$uploadstime;
	//为上传的文件新起一个名字，保证更加安全
	//$new_filename = date('YmdHis',time()).rand(100,1000).'.'.$ext_suffix;
	//将文件从临时路径移动到磁盘
	//if (move_uploaded_file($temp_name, 'uploads/'.$filename)){
	if (move_uploaded_file($temp_name, $time_dir."/".$filename)){
